...
from senteval.sick import SICKRelatednessEval, MySICKRelatednessEval, SICKEntailmentEval
from senteval.mrpc import MRPCEval
from senteval.sts import STS12Eval, STS13Eval, STS14Eval, STS15Eval, STS16Eval, STSBenchmarkEval, MySTSBenchmarkEval
from senteval.sst import SSTEval
from senteval.afs import AFSEval
...
class SE(object):
    def __init__(self, params, batcher, prepare=None):
        ...
        self.list_tasks = ['CR', 'MR', 'MPQA', 'SUBJ', 'SST2', 'SST5', 'TREC', 'MRPC',
                           'SICKRelatedness', 'MySICKRelatedness',
                           'SICKEntailment', 'STSBenchmark', 'MySTSBenchmark',
                           'SNLI', 'ImageCaptionRetrieval', 'STS12', 'STS13',
                           'STS14', 'STS15', 'STS16', 'AFS',
    ...
    def eval(self, name):
        ...
        elif name == 'MySICKRelatedness':
            self.evaluation = MySICKRelatednessEval(tpath + '/downstream/SICK', seed=self.params.seed)
        elif name == 'STSBenchmark':
            self.evaluation = STSBenchmarkEval(tpath + '/downstream/STS/STSBenchmark', seed=self.params.seed)
        elif name == 'MySTSBenchmark':
            self.evaluation = MySTSBenchmarkEval(tpath + '/downstream/STS/STSBenchmark', seed=self.params.seed)
        elif name == 'SICKEntailment':
            self.evaluation = SICKEntailmentEval(tpath + '/downstream/SICK', seed=self.params.seed)
        elif name == 'AFS':
            self.evaluation = AFSEval(tpath + '/downstream/AFS', seed=self.params.seed)
        elif name == 'SNLI':
